...
Database.prototype.read = function(collectionName, cb) {
  ...
  fs.exists(filePath, function(exists) {
    ...
    if(Object.keys(self.data[filePath]).length !== 0) return cb(null, {
...
Database.prototype.dropCollection = function(collectionName, relations, cb) {
  ...
  relations.forEach(function(relation) {
    delete self.data[filePath][relation];
    delete self.schema[filePath][relation];
    delete self.counters[filePath][relation];
  });
...
Database.prototype.insert = function(collectionName, values, cb) {
  ...
  if (!this.data[filePath][name]) return cb(Err.adapter.collectionNotRegistered);
...
Database.prototype.update = function(collectionName, options, values, cb) {
  ...
  var uniqueAttrs = _.where(_.keys(values), function(attrName) {
    var attributeDef = self.schema[filePath][name][attrName];
  ...
  if (uniqueAttrs.length && resultSet.indicies.length) {
    ...
    // we're updating it to, so that there wouldn't be more than one record with the same value.
      ...
      var result = self.data[filePath][name][resultSet.indicies[0]];
      var errors = [];
...
Database.prototype.serializeValues = function(collectionName, values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    if(!self.schema[filePath][name]) return;
    if(!self.schema[filePath][name].hasOwnProperty(key)) return;

    var type = self.schema[filePath][name][key].type,
        val;
