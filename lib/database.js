...
Database.prototype.setCollection = function(collectionName, options) {
  var name = collectionName.toLowerCase();
...
Database.prototype.getCollection = function(collectionName) {
  ...
  var data = this.data[filePath][name];
  var schema = this.schema[filePath][name];
  var counters = this.counters[filePath][name];
...
Database.prototype.createCollection = function(collectionName, definition, cb) {
  var collection = this.setCollection(collectionName, { definition: definition });

  this.write(collectionName, function(err) {
    if(err) return cb(err);
...
Database.prototype.describe = function(collectionName, cb) {
  ...
  this.read(collectionName, function(err, data) {
    if(err) return cb(err);
    var schema = data.schema[name] || null;
    cb(null, schema);
  });
};
