...
Database.prototype.select = function(collectionName, options, cb) {
  ...
  var dateTimeKeys = [];
  for (var key in this.schema[collectionName]) {
    var attr = this.schema[collectionName][key];
    if (attr.type === 'datetime') {
      dateTimeKeys.push(key);
    }
  }

  var results = aggregate.results.map(function(item) {
    dateTimeKeys.forEach(function(key) {
